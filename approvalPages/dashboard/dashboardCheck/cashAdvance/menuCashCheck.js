...
async function loadDashboard() {
    try {
        ...
        if (currentTab === 'prepared') {
            url = `${BASE_URL}/api/cash-advance/dashboard/approval?ApproverId=${userId}&ApproverRole=checked&isApproved=false`;
        } else if (currentTab === 'checked') {
            url = `${BASE_URL}/api/cash-advance/dashboard/approval?ApproverId=${userId}&ApproverRole=checked&isApproved=true`;
        } else if (currentTab === 'rejected') {
            url = `${BASE_URL}/api/cash-advance/dashboard/rejected?ApproverId=${userId}&ApproverRole=checked`;
        }
...
async function updateCounters(userId) {
    try {
        ...
        const preparedResponse = await fetch(`${BASE_URL}/api/cash-advance/dashboard/approval?ApproverId=${userId}&ApproverRole=checked&isApproved=false`, {
            headers: { 'Authorization': `Bearer ${getAccessToken()}` }
        });
        const checkedResponse = await fetch(`${BASE_URL}/api/cash-advance/dashboard/approval?ApproverId=${userId}&ApproverRole=checked&isApproved=true`, {
            headers: { 'Authorization': `Bearer ${getAccessToken()}` }
        });
        const rejectedResponse = await fetch(`${BASE_URL}/api/cash-advance/dashboard/rejected?ApproverId=${userId}&ApproverRole=checked`, {
            headers: { 'Authorization': `Bearer ${getAccessToken()}` }
        });
...
function downloadExcel() {
    ...
    // Save file
    XLSX.writeFile(wb, 'Cash_Advances_Check.xlsx');
}
...
function downloadPDF() {
    ...
    doc.text('Cash Advances Check Report', 14, 16);
    doc.autoTable({
    ...
    // Save file
    doc.save('Cash_Advances_Check.pdf');
} 