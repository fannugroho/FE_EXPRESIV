...
async function loadDashboard() {
    try {
        ...
        if (currentTab === 'prepared') {
            url = `${BASE_URL}/api/settlements/dashboard/approval?ApproverId=${userId}&ApproverRole=checked&isApproved=false`;
        } else if (currentTab === 'checked') {
            url = `${BASE_URL}/api/settlements/dashboard/approval?ApproverId=${userId}&ApproverRole=checked&isApproved=true`;
        } else if (currentTab === 'rejected') {
            url = `${BASE_URL}/api/settlements/dashboard/rejected?ApproverId=${userId}&ApproverRole=checked`;
        }
...
async function updateCounters(userId) {
    try {
        ...
        const preparedResponse = await fetch(`${BASE_URL}/api/settlements/dashboard/approval?ApproverId=${userId}&ApproverRole=checked&isApproved=false`, {
            headers: { 'Authorization': `Bearer ${getAccessToken()}` }
        });
        const checkedResponse = await fetch(`${BASE_URL}/api/settlements/dashboard/approval?ApproverId=${userId}&ApproverRole=checked&isApproved=true`, {
            headers: { 'Authorization': `Bearer ${getAccessToken()}` }
        });
        const rejectedResponse = await fetch(`${BASE_URL}/api/settlements/dashboard/rejected?ApproverId=${userId}&ApproverRole=checked`, {
            headers: { 'Authorization': `Bearer ${getAccessToken()}` }
        });
...
function detailSettle(settleId) {
    window.location.href = `../../../approval/check/settlement/checkedSettle.html?settle-id=${settleId}&tab=${currentTab}`;
}
...
function downloadExcel() {
    ...
    // Save file
    XLSX.writeFile(wb, 'Settlements_Check.xlsx');
}
...
function downloadPDF() {
    ...
    doc.text('Settlements Check Report', 14, 16);
    doc.autoTable({
    ...
    // Save file
    doc.save('Settlements_Check.pdf');
} 