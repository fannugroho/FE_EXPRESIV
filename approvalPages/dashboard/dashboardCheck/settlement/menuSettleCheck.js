        ...
        searchInput.addEventListener('input', function() {
            ...
            if (searchTerm === '') {
                ...
                const tabData = currentTab === 'all' 
                    ? allSettlements 
                    : allSettlements.filter(item => item.status.toLowerCase() === currentTab);
...
function switchTab(tabName) {
    ...
    if (tabName === 'all') {
        ...
        filteredData = allSettlements;
    } else if (tabName === 'draft') {
        document.getElementById('draftTabBtn').classList.add('tab-active');
        filteredData = allSettlements.filter(item => item.status === 'Draft');
        ...
        filteredData = allSettlements.filter(item => item.status === 'Paid');
    } else if (tabName === 'close') {
        filteredData = allSettlements.filter(item => item.status === 'Close');
