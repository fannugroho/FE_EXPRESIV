...
async function updateCounters(userId) {
    try {
        ...
        const revisionResponse = await fetch(`${BASE_URL}/api/cash-advance/dashboard/revision?filterType=revision&userId=${userId}`, {
...
async function filterAndDisplayDocuments(userId) {
    try {
        ...
        if (currentTab === 'revision') {
            url = `${BASE_URL}/api/cash-advance/dashboard/revision?filterType=revision&userId=${userId}`;
