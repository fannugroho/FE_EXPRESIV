...
async function loadDashboard() {
    try {
        ...
        if (currentTab === 'acknowledge') {
            baseUrl = `${BASE_URL}/api/invoice/dashboard/approval`;
...
async function updateCounters() {
    try {
        ...
        const acknowledgeCount = allInvoices.filter(inv => inv.status === 'acknowledge').length;
        ...
        // In production, uncomment this:
        /*
        const response = await fetch(`${BASE_URL}/api/invoice/dashboard/counters?ApproverId=${userId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${getAccessToken()}`
        ...
        if (response.ok) {
            ...
            document.getElementById('acknowledgeCount').textContent = data.acknowledge || 0;
            document.getElementById('approveCount').textContent = data.approved || 0;
