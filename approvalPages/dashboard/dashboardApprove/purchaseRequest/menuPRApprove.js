...
// Notifikasi dokumen yang perlu di-approve (acknowledge)
                ...
                <div class="flex items-start justify-between">
                    <div class="flex-1">
                        ...
                        <div class="inline-block mt-1">
                            <span class="px-2 py-1 text-xs rounded-full bg-yellow-100 text-yellow-800">${data.status || 'Acknowledge'}</span>
                        </div>
                    </div>
...
function showNotification(message, prNumber) {
    ...
    const data = {
        ...
        status: message.split('-')[4] || 'Acknowledge'
    };
...
// Polling untuk dokumen yang sudah acknowledge (perlu di-approve)
...
// Polling untuk dokumen yang sudah approved
async function pollApprovedDocs() {
    try {
        const userId = getUserId();
        if (!userId) return;
        const response = await fetch(`${BASE_URL}/api/pr/dashboard/approval?ApproverId=${userId}&ApproverRole=approved&isApproved=true`, {
            headers: { 'Authorization': `Bearer ${getAccessToken()}` }
        });
        const data = await response.json();
        const approvedPRs = new Set((data.data || []).map(doc => doc.purchaseRequestNo));
        // Hapus notifikasi untuk PR yang sudah approved
        notifiedPRs.forEach(prNumber => {
            if (approvedPRs.has(prNumber)) {
...
async function pollRejectedDocs() {
    try {
        const userId = getUserId();
        if (!userId) return;
        const response = await fetch(`${BASE_URL}/api/pr/dashboard/rejected?ApproverId=${userId}&ApproverRole=approved`, {
            headers: { 'Authorization': `Bearer ${getAccessToken()}` }
        });
        const data = await response.json();
