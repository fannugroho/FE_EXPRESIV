...
async function downloadExcel() {
    try {
        ...
        XLSX.writeFile(workbook, `purchase_request_receive_${statusText.toLowerCase()}_${new Date().toISOString().split('T')[0]}.xlsx`);
    } catch (error) {
...
async function downloadPDF() {
    try {
        ...
        doc.setFontSize(16);
        doc.text(`Purchase Request Receive Report - ${statusText}`, 14, 15);
        ...
        doc.save(`purchase_request_receive_${statusText.toLowerCase()}_${new Date().toISOString().split('T')[0]}.pdf`);
    } catch (error) {
...
// Notifikasi dokumen yang perlu di-receive (approved)
...
// Polling untuk dokumen yang sudah approved (perlu di-receive)
async function pollApprovedDocs() {
    try {
        ...
        console.warn('Error polling approved docs:', e);
    }
}

// Polling untuk dokumen yang sudah received
async function pollReceivedDocs() {
    try {
        const userId = getUserId();
        if (!userId) return;
        const response = await fetch(`${BASE_URL}/api/pr/dashboard/approval?ApproverId=${userId}&ApproverRole=received&isApproved=true`, {
            headers: { 'Authorization': `Bearer ${getAccessToken()}` }
        });
        const data = await response.json();
        const receivedPRs = new Set((data.data || []).map(doc => doc.purchaseRequestNo));
        // Hapus notifikasi untuk PR yang sudah received
        notifiedPRs.forEach(prNumber => {
            if (receivedPRs.has(prNumber)) {
                removeNotification(prNumber);
            }
        });
    } catch (e) {
        console.warn('Error polling received docs:', e);
    }
}
...
async function pollRejectedDocs() {
    try {
        const userId = getUserId();
        if (!userId) return;
        const response = await fetch(`${BASE_URL}/api/pr/dashboard/rejected?ApproverId=${userId}&ApproverRole=received`, {
            headers: { 'Authorization': `Bearer ${getAccessToken()}` }
        });
        const data = await response.json();
