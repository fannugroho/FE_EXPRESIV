...
async function updateCounters(userId) {
    try {
        ...
        const checkedResponse = await fetch(`${BASE_URL}/api/pr/dashboard/approval?ApproverId=${userId}&ApproverRole=acknowledged&isApproved=false`, {
            headers: { 'Authorization': `Bearer ${getAccessToken()}` }
        });
        const acknowledgedResponse = await fetch(`${BASE_URL}/api/pr/dashboard/approval?ApproverId=${userId}&ApproverRole=acknowledged&isApproved=true`, {
            headers: { 'Authorization': `Bearer ${getAccessToken()}` }
        });
        const rejectedResponse = await fetch(`${BASE_URL}/api/pr/dashboard/rejected?ApproverId=${userId}&ApproverRole=acknowledged`, {
            headers: { 'Authorization': `Bearer ${getAccessToken()}` }
        });
...
function detailDoc(id, prType) {
    window.location.href = `../../../approval/acknowledge/purchaseRequest/acknowledgePR.html?pr-id=${id}&pr-type=${prType}&tab=${currentTab}`;
}
...
async function downloadExcel() {
    try {
        ...
        XLSX.writeFile(workbook, `purchase_request_acknowledge_${statusText.toLowerCase()}_${new Date().toISOString().split('T')[0]}.xlsx`);
    } catch (error) {
...
async function downloadPDF() {
    try {
        ...
        doc.setFontSize(16);
        doc.text(`Purchase Request Acknowledge Report - ${statusText}`, 14, 15);
        ...
        doc.save(`purchase_request_acknowledge_${statusText.toLowerCase()}_${new Date().toISOString().split('T')[0]}.pdf`);
    } catch (error) {
...
// Notifikasi dokumen yang perlu di-acknowledge (checked)
...
// Polling untuk dokumen yang sudah checked (perlu di-acknowledge)
...
// Polling untuk dokumen yang sudah acknowledged
async function pollAcknowledgedDocs() {
    try {
        const userId = getUserId();
        if (!userId) return;
        const response = await fetch(`${BASE_URL}/api/pr/dashboard/approval?ApproverId=${userId}&ApproverRole=acknowledged&isApproved=true`, {
            headers: { 'Authorization': `Bearer ${getAccessToken()}` }
        });
        const data = await response.json();
        const acknowledgedPRs = new Set((data.data || []).map(doc => doc.purchaseRequestNo));
        // Hapus notifikasi untuk PR yang sudah acknowledged
        notifiedPRs.forEach(prNumber => {
            if (acknowledgedPRs.has(prNumber)) {
                removeNotification(prNumber);
            }
        });
    } catch (e) {
        console.warn('Error polling acknowledged docs:', e);
    }
}

// Polling untuk dokumen yang rejected
async function pollRejectedDocs() {
    try {
        const userId = getUserId();
        if (!userId) return;
        const response = await fetch(`${BASE_URL}/api/pr/dashboard/rejected?ApproverId=${userId}&ApproverRole=acknowledged`, {
            headers: { 'Authorization': `Bearer ${getAccessToken()}` }
        });
        const data = await response.json();
        const rejectedPRs = new Set((data.data || []).map(doc => doc.purchaseRequestNo));
        // Hapus notifikasi untuk PR yang sudah rejected
        notifiedPRs.forEach(prNumber => {
            if (rejectedPRs.has(prNumber)) {
                removeNotification(prNumber);
            }
        });
    } catch (e) {
        console.warn('Error polling rejected docs:', e);
    }
}
