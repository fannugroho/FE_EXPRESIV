// ===== CLEAN CHECK OUTGOING PAYMENT REIMBURSEMENT SYSTEM =====
// File: checkedOPReim.js

// ===== 1. GLOBAL STATE MANAGEMENT =====
class OPReimCheckState {
    constructor() {
...
const checkState = new OPReimCheckState();
...
class FormManager {
    static populateFormFields(data) {
        ...
        UIUtils.setElementValue('RemittanceRequestAmount', UIUtils.formatCurrency(data.remittanceRequestAmount || 0));
        UIUtils.setElementValue('TrsfrAcct', data.trsfrAcct || '');
        UIUtils.setElementValue('TrsfrSum', UIUtils.formatCurrency(data.trsfrSum || 0));
        ...
        if (data.taxDate) {
            UIUtils.setElementValue('TaxDate', UIUtils.formatDateSafely(data.taxDate));
        }
        if (data.trsfrDate) {
...
class UserManager {
    static getUserNameById(userId) {
        if (!checkState.usersList || !userId) return 'Unknown User';

        const user = checkState.usersList.find(u => u.id === userId);
...
class PermissionManager {
    static checkUserPermissions(data) {
        ...
        const isAboveChecker = this.isUserAboveChecker(currentUser.userId, approval.checkedBy);

        console.log('Permission check:', {
            currentStatus,
            currentUserId: currentUser.userId,
            checkedById: approval.checkedBy,
            isAssignedChecker,
            isAboveChecker
        });

        this.hideButtonsBasedOnStatus(data);
        this.updateButtonStates(currentStatus, isAssignedChecker, isAboveChecker, approval);
    }
    ...
    static updateButtonStates(currentStatus, isAssignedChecker, isAboveChecker, approval) {
        const approveButton = document.getElementById('approveButton');
        const rejectButton = document.getElementById('rejectButton');

        if (currentStatus === 'Prepared' && isAssignedChecker) {
            this.enableButtons(approveButton, rejectButton);
            UIUtils.showInfo('Ready for Checking', 'You can now check this document');
        } else if (currentStatus === 'Prepared' && isAboveChecker) {
            this.disableButtons(approveButton, rejectButton);
            const checkerName = UserManager.getUserNameById(approval.checkedBy);
            UIUtils.showWarning('Document Pending', `Please wait for ${checkerName} to check this document first`);
    ...
    static isUserAboveChecker(currentUserId, checkerId) {
        return currentUserId !== checkerId;
    }

    static getStatusMessage(status) {
        const messages = {
            'Checked': 'This document has already been checked.',
            'Acknowledged': 'This document has been acknowledged.',
            'Approved': 'This document has been approved.',
            'Received': 'This document has been received.',
            'Rejected': 'This document has been rejected.'
        };
        return messages[status] || 'This document is not ready for checking.';
    }
    ...
    static validateDocumentStatus() {
        ...
        if (!checkState.opReimData || !checkState.opReimData.approval) {
            UIUtils.showError('Error', 'Document data is incomplete. Please refresh the page.');
            return false;
        }

        const approval = checkState.opReimData.approval;

        if (approval.checkedDate) {
            UIUtils.showInfo('Already Checked', 'This document has already been checked.');
            return false;
        }

        if (approval.checkedBy !== currentUser.userId) {
            const checkerName = UserManager.getUserNameById(approval.checkedBy);
            UIUtils.showWarning('Not Authorized', `Only ${checkerName} can check this document.`);
            return false;
        }
...
class AttachmentManager {
    ...
    static async viewEnhancedAttachment(attachmentOrPath, fileName) {
        try {
            ...
            const docId = checkState.documentId;
            if (!docId) {
...
class DataManager {
    static async initialize() {
        try {
            console.log('🚀 Initializing Check Outgoing Payment Reimbursement page...');
            ...
            checkState.setDocumentId(documentId);
    ...
    static async loadUsersData() {
        try {
            ...
            checkState.setUsers(usersData.data || []);
        } catch (error) {
...
// ===== 12. ACTION MANAGER =====
class ActionManager {
    static async approveOPReim() {
        try {
            if (!PermissionManager.validateDocumentStatus()) {
                return;
            }

            UIUtils.showLoading('Processing...', 'Submitting approval');
            ...
            const requestData = this.buildApprovalRequestData(userId, currentUserName, currentDate);

            const response = await fetch(`${BASE_URL}/api/staging-outgoing-payments/approvals/${checkState.documentId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json-patch+json' },
                body: JSON.stringify(requestData)
            });
            ...
            UIUtils.showSuccess('Success', 'Document has been checked successfully')
                .then(() => DataManager.goToMenu());

        } catch (error) {
            console.error('❌ Error checking document:', error);
            UIUtils.showError('Error', `Failed to check document: ${error.message}`);
        }
    }

    static buildApprovalRequestData(userId, currentUserName, currentDate) {
        const approval = checkState.opReimData.approval || {};
        ...
        return {
            ...
            preparedBy: approval.preparedBy || userId,
            checkedBy: userId,
            acknowledgedBy: approval.acknowledgedBy || userId,
            approvedBy: approval.approvedBy || userId,
            receivedBy: approval.receivedBy || userId,
            preparedDate: approval.preparedDate || currentDate,
            preparedByName: approval.preparedByName || currentUserName,
            checkedByName: currentUserName,
            acknowledgedByName: approval.acknowledgedByName || currentUserName,
            approvedByName: approval.approvedByName || currentUserName,
            receivedByName: approval.receivedByName || currentUserName,
    ...
    static buildRejectionRequestData(userId, rejectionReason) {
        ...
        return {
            stagingID: checkState.documentId,
            createdAt: checkState.opReimData.createdAt || currentDate,
            updatedAt: currentDate,
...
function approveOPReim() {
    ActionManager.approveOPReim();
}

function rejectOPReim() {
    ActionManager.rejectOPReim();
}
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🚀 Initializing Check Outgoing Payment Reimbursement System...');
    DataManager.initialize();
    console.log('✅ Check Outgoing Payment Reimbursement System initialized successfully');
});
