    ...
    try {
        ...
        if (isDevelopmentMode) {
            ...
            const dummyData = {
                ...
                VatSum: 100000,
                DocTotal: 1100000,
                Comments: "Sample invoice item for approval",
                items: [
                    {
                        LineNum: 0,
                        ItemCode: "ITEM001",
            ...
            const response = await fetch(`${BASE_URL}/api/invoice-items/${documentId}`);
...
async function approveInvItem() {
    try {
        ...
        if (result.isConfirmed) {
            ...
            const response = await fetch(`${BASE_URL}/api/invoice-items/approve`, {
                method: 'POST',
                headers: {
            ...
            Swal.fire({
                icon: 'success',
                title: 'Success!',
                text: 'Invoice item has been approved successfully.',
                timer: 2000,
        ...
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Failed to approve invoice item'
        });
    }
}
...
async function rejectInvItem() {
    try {
        ...
        if (rejectionReason) {
            ...
            const response = await fetch(`${BASE_URL}/api/invoice-items/reject`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
...
function goToMenuApproveInvItem() {
    window.location.href = '../../dashboard/dashboardApprove/invoiceItem/menuInvItemApprove.html';
}
