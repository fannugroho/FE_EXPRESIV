// ===== CLEAN ACKNOWLEDGE OUTGOING PAYMENT REIMBURSEMENT SYSTEM =====
// File: acknowledgeOPReim.js

// ===== 1. GLOBAL STATE MANAGEMENT =====
class OPReimAcknowledgeState {
    constructor() {
...
const acknowledgeState = new OPReimAcknowledgeState();
...
class OPReimAPIService {
    ...
    static async acknowledgeDocument(id, requestData) {
...
class UserManager {
    static getUserNameById(userId) {
        if (!acknowledgeState.usersList || !userId) return 'Unknown User';

        const user = acknowledgeState.usersList.find(u => u.id === userId);
...
class PermissionManager {
    static checkUserPermissions(data) {
        ...
        const isAboveAcknowledger = this.isUserAboveAcknowledger(currentUser.userId, approval.acknowledgedBy);

        console.log('Permission check:', {
            currentStatus,
            currentUserId: currentUser.userId,
            acknowledgedById: approval.acknowledgedBy,
            isAssignedAcknowledger,
            isReadyForAcknowledging,
            isAboveAcknowledger
        });

        this.hideButtonsBasedOnStatus(data);
        this.updateButtonStates(currentStatus, isAssignedAcknowledger, isReadyForAcknowledging, approval);
    }
    ...
    static updateButtonStates(currentStatus, isAssignedAcknowledger, isReadyForAcknowledging, approval) {
        const acknowledgeButton = document.querySelector('button[onclick="acknowledgeOPReim()"]') || document.getElementById('acknowledgeButton');
        const rejectButton = document.querySelector('button[onclick="rejectOPReim()"]') || document.getElementById('rejectButton');

        if (currentStatus === 'Checked' && isAssignedAcknowledger) {
            this.enableButtons(acknowledgeButton, rejectButton);
            UIUtils.showInfo('Ready for Acknowledging', 'You can now acknowledge this document');
        } else if (currentStatus === 'Checked' && !isAssignedAcknowledger) {
            this.disableButtons(acknowledgeButton, rejectButton);
            const acknowledgerName = UserManager.getUserNameById(approval.acknowledgedBy);
            UIUtils.showWarning('Document Pending', `Only ${acknowledgerName} can acknowledge this document`);
    ...
    static enableButtons(acknowledgeButton, rejectButton) {
    ...
    static disableButtons(acknowledgeButton, rejectButton) {
        if (acknowledgeButton) {
            acknowledgeButton.disabled = true;
            acknowledgeButton.classList.add('opacity-50', 'cursor-not-allowed');
        }
        if (rejectButton) {
    ...
    static hideButtonsBasedOnStatus(data) {
        ...
        // Show buttons only when ready for acknowledging (Checked status)
    ...
    static isUserAboveAcknowledger(currentUserId, acknowledgerId) {
        return currentUserId !== acknowledgerId;
    }

    static getStatusMessage(status) {
        const messages = {
            'Prepared': 'This document is prepared and waiting to be checked.',
            'Checked': 'This document has been checked and is ready for acknowledging.',
            'Acknowledged': 'This document has already been acknowledged.',
            'Approved': 'This document has been approved.',
            'Received': 'This document has been received.',
            'Rejected': 'This document has been rejected.'
        };
        return messages[status] || 'This document is not ready for acknowledging.';
    }
    ...
    static validateDocumentStatus() {
        ...
        if (!acknowledgeState.opReimData || !acknowledgeState.opReimData.approval) {
            UIUtils.showError('Error', 'Document data is incomplete. Please refresh the page.');
            return false;
        }

        const approval = acknowledgeState.opReimData.approval;

        if (approval.acknowledgedDate) {
            UIUtils.showInfo('Already Acknowledged', 'This document has already been acknowledged.');
            return false;
        }

        if (approval.acknowledgedBy !== currentUser.userId) {
            const acknowledgerName = UserManager.getUserNameById(approval.acknowledgedBy);
            UIUtils.showWarning('Not Authorized', `Only ${acknowledgerName} can acknowledge this document.`);
            return false;
        }

        if (!approval.checkedDate) {
            UIUtils.showWarning('Not Ready', 'This document must be checked before it can be acknowledged.');
            return false;
        }
...
class AttachmentManager {
    ...
    static async viewEnhancedAttachment(attachmentOrPath, fileName) {
        try {
            ...
            const docId = acknowledgeState.documentId;
            if (!docId) {
...
class DataManager {
    static async initialize() {
        try {
            ...
            acknowledgeState.setDocumentId(documentId);
    ...
    static async loadOPReimDetails(id) {
        try {
            ...
            acknowledgeState.setOPReimData(data);
    ...
    static async loadUsersData() {
        try {
            ...
            acknowledgeState.setUsers(usersData.data || []);
        } catch (error) {
            console.error('❌ Error loading users:', error);
            acknowledgeState.setUsers([]);
        }
    }
...
class ActionManager {
    static async acknowledgeOPReim() {
        try {
            ...
            const response = await fetch(`${BASE_URL}/api/staging-outgoing-payments/approvals/${acknowledgeState.documentId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json-patch+json' },
                body: JSON.stringify(requestData)
            });
    ...
    static buildAcknowledgeRequestData(userId, currentUserName, currentDate) {
        ...
        return {
            stagingID: acknowledgeState.documentId,
    ...
    static buildRejectionRequestData(userId, rejectionReason) {
        ...
        return {
            stagingID: acknowledgeState.documentId,
            createdAt: acknowledgeState.opReimData.createdAt || currentDate,
            updatedAt: currentDate,
...
function acknowledgeOPReim() {
    ActionManager.acknowledgeOPReim();
}
