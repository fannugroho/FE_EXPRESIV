...
console.log('menuARInvoiceCloneCheck.js loaded');
...
async function fetchARInvoiceCloneDocuments(step, userId, onlyCurrentStep = false, isRejected = false) {
    try {
        ...
        const apiUrl = `${BASE_URL}/api/staging-ar-invoice-clone/headers?${params.toString()}`;
...
async function fetchAllDocuments(userId) {
    try {
        ...
        const promises = steps.map(step => fetchARInvoiceCloneDocuments(step, userId, false));
...
async function fetchCheckedDocuments(userId) {
    ...
    try {
        ...
        const allDocuments = await fetchARInvoiceCloneDocuments('checkedBy', userId, false);
...
async function debugAllDocuments(userId) {
    ...
    try {
        // Get all documents from checkedBy endpoint
        const allDocs = await fetchARInvoiceCloneDocuments('checkedBy', userId, false);
...
function detailDoc(id) {
    // Navigate to AR Invoice detail page using relative path
    const detailUrl = `../../../Modul_ARinvoice/0_DetailCreated/detailARInvoiceNew.html?id=${id}&tab=${currentTab}`;
    
    console.log('=== detailDoc Debug Info (Clone) ===');
...
function downloadExcel() {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'AR Invoice Clone');

    // Generate Excel file
    XLSX.writeFile(workbook, `ar_invoice_clone_${statusText.toLowerCase()}_${new Date().toISOString().split('T')[0]}.xlsx`);
}
...
function downloadPDF() {
    ...
    doc.setFontSize(16);
    doc.text(`AR Invoice Clone Report - ${statusText}`, 14, 15);
