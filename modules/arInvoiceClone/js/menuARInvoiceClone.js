// Variabel untuk menyimpan dokumen AR Invoice Clone
let arInvoiceCloneDocs = [];
let allARInvoiceCloneDocs = []; // Menyimpan semua dokumen sebelum filtering
...
// Reusable function to fetch AR Invoice Clone documents by approval step
async function fetchARInvoiceCloneDocuments(step, userId, onlyCurrentStep = false) {
    try {
        ...
        const response = await fetch(`${BASE_URL}/api/staging-ar-invoice-clone/headers?${params.toString()}`, {
            method: 'GET',
            headers: {
...
async function fetchAllDocuments(userId) {
    try {
        ...
        return await fetchARInvoiceCloneDocuments('preparedBy', userId, false);
    } catch (error) {
...
async function fetchPreparedDocuments(userId) {
    try {
        ...
        const allDocuments = await fetchARInvoiceCloneDocuments('preparedBy', userId, false);
...
// Fungsi untuk menampilkan modal AR Invoice Clone
async function showARInvoiceCloneModal() {
    // Ambil data AR Invoice Clone
    await fetchARInvoiceCloneDocs();
...
// Fungsi untuk menutup modal AR Invoice Clone
function closeARInvoiceCloneModal() {
    const modal = document.getElementById('arInvoiceCloneModal');
...
async function fetchARInvoiceCloneDocs() {
    const tableBody = document.getElementById("arInvoiceCloneDocs");
    ...
    try {
        ...
        const response = await fetch(`${BASE_URL}/api/ar-invoice-clone?Status=received`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'accept': '*/*'
        ...
        if (data.status && data.data) {
            ...
            allARInvoiceCloneDocs = data.data;
            // Filter hanya dokumen dengan transferOutgoing = "O"
            arInvoiceCloneDocs = allARInvoiceCloneDocs.filter(doc => doc.transferOutgoing === "O");
            console.log(`Filtered ${arInvoiceCloneDocs.length} documents with transferOutgoing="O" from ${allARInvoiceCloneDocs.length} total documents`);
            displayARInvoiceCloneDocs(arInvoiceCloneDocs);
            
            if (arInvoiceCloneDocs.length === 0) {
...
function displayARInvoiceCloneDocs(docs) {
    const tableBody = document.getElementById("arInvoiceCloneDocs");
    tableBody.innerHTML = "";
        ...
        const row = `<tr class='border-b hover:bg-gray-100'>
            ...
            <td class='p-2'>
                <div class="flex space-x-1">
                    <button onclick="selectARInvoiceClone('${doc.id || ''}', '${doc.voucherNo || ''}')" class="bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-600 text-xs">Select</button>
                </div>
            </td>
        </tr>`;
...
// Fungsi untuk memfilter dokumen AR Invoice Clone berdasarkan pencarian
function filterARInvoiceCloneDocs() {
    const searchTerm = document.getElementById('arInvoiceCloneSearchInput').value.toLowerCase();
    ...
    const filteredDocs = arInvoiceCloneDocs.filter(doc => {
    ...
    console.log(`Search filtered ${filteredDocs.length} documents from ${arInvoiceCloneDocs.length} transferOutgoing="O" documents`);
    displayARInvoiceCloneDocs(filteredDocs);
}

// Fungsi untuk memilih dokumen dan membuat AR Invoice Clone
function selectARInvoiceClone(docId, voucherNo) {
    // Tampilkan dialog konfirmasi
    Swal.fire({
        title: 'Create AR Invoice Clone',
        text: `Do you want to create a new AR Invoice Clone based on document "${voucherNo}"?`,
        icon: 'question',
        ...
        if (result.isConfirmed) {
            // Redirect ke halaman pembuatan AR Invoice Clone dengan parameter document ID
            window.location.href = `../addPages/arInvoiceClone/addARInvoiceClone.html?document-id=${docId}`;
        }
    });
...
async function loadDashboard() {
    ...
    try {
        ...
        const summaryResponse = await fetch(`${BASE_URL}/api/staging-ar-invoice-clone/dashboard/summary`);
...
function goToCheckedDocs() {
    ...
    debugNavigation('goToCheckedDocs', 'approvalPages/dashboard/dashboardCheck/ARInvoiceClone/menuARInvoiceCloneCheck.html');
    if (typeof navigateToPage === 'function') {
        navigateToPage('approvalPages/dashboard/dashboardCheck/ARInvoiceClone/menuARInvoiceCloneCheck.html');
    } else {
        window.location.href = 'approvalPages/dashboard/dashboardCheck/ARInvoiceClone/menuARInvoiceCloneCheck.html';
    }
}

function goToApprovedDocs() {
    // Navigate to the approved documents page
    debugNavigation('goToApprovedDocs', 'approvalPages/dashboard/dashboardApprove/ARInvoiceClone/menuARInvoiceCloneApprove.html');
    if (typeof navigateToPage === 'function') {
        navigateToPage('approvalPages/dashboard/dashboardApprove/ARInvoiceClone/menuARInvoiceCloneApprove.html');
    } else {
        window.location.href = 'approvalPages/dashboard/dashboardApprove/ARInvoiceClone/menuARInvoiceCloneApprove.html';
    }
}

function goToRejectDocs() {
    // Navigate to the rejected documents page (same as approved but filtered)
    debugNavigation('goToRejectDocs', 'approvalPages/dashboard/dashboardApprove/ARInvoiceClone/menuARInvoiceCloneApprove.html');
    if (typeof navigateToPage === 'function') {
        navigateToPage('approvalPages/dashboard/dashboardApprove/ARInvoiceClone/menuARInvoiceCloneApprove.html');
    } else {
        window.location.href = 'approvalPages/dashboard/dashboardApprove/ARInvoiceClone/menuARInvoiceCloneApprove.html';
...
function goToPaidDocs() {
    ...
    debugNavigation('goToPaidDocs', 'approvalPages/dashboard/dashboardApprove/ARInvoiceClone/menuARInvoiceCloneApprove.html');
    if (typeof navigateToPage === 'function') {
        navigateToPage('approvalPages/dashboard/dashboardApprove/ARInvoiceClone/menuARInvoiceCloneApprove.html');
    } else {
        window.location.href = 'approvalPages/dashboard/dashboardApprove/ARInvoiceClone/menuARInvoiceCloneApprove.html';
...
function goToSettledDocs() {
    ...
    debugNavigation('goToSettledDocs', 'approvalPages/dashboard/dashboardApprove/ARInvoiceClone/menuARInvoiceCloneApprove.html');
    if (typeof navigateToPage === 'function') {
        navigateToPage('approvalPages/dashboard/dashboardApprove/ARInvoiceClone/menuARInvoiceCloneApprove.html');
    } else {
        window.location.href = 'approvalPages/dashboard/dashboardApprove/ARInvoiceClone/menuARInvoiceCloneApprove.html';
...
function downloadExcel() {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "AR Invoice Clone");
    
    // Generate Excel file and trigger download
    const fileName = `AR_Invoice_Clone_${new Date().toISOString().split('T')[0]}.xlsx`;
...
function downloadPDF() {
    ...
    doc.setProperties({
        title: 'AR Invoice Clone Report'
    });
    
    // Add title
    doc.setFontSize(18);
    doc.text('AR Invoice Clone Report', 14, 22);
    ...
    // Save PDF
    const fileName = `AR_Invoice_Clone_${new Date().toISOString().split('T')[0]}.pdf`;
    doc.save(fileName);
}
...
function logout() { 
    ...
    localStorage.removeItem("hasARInvoiceCloneAccess");
...
function detailDoc(opId) {
    // Navigate to AR Invoice Clone detail page
    debugNavigation('detailDoc', `detailPages/arInvoiceClone/detailARInvoiceClone.html?id=${opId}`);
    if (typeof navigateToPage === 'function') {
        navigateToPage(`detailPages/arInvoiceClone/detailARInvoiceClone.html?id=${opId}`);
    } else {
        window.location.href = `detailPages/arInvoiceClone/detailARInvoiceClone.html?id=${opId}`;
    }
}
