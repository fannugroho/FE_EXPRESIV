...
async function viewAttachment(attachmentOrPath, fileName) {
    try {
        ...
        let response = await makeAuthenticatedRequest(`/api/staging-ar-invoice-clone/attachments/${docId}`, {
            method: 'GET',
            headers: {
        ...
        if (!response.ok) {
            ...
            if (response.status === 405) {
                ...
                response = await makeAuthenticatedRequest(`/api/staging-ar-invoice-clone/headers/${docId}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json'
                    }
...
async function loadDocumentData() {
    ...
    if (docId) {
        ...
        try {
            ...
            console.log('🌐 API Request:', `GET /api/staging-ar-invoice-clone/headers/${docId}`);

            const response = await makeAuthenticatedRequest(`/api/staging-ar-invoice-clone/headers/${docId}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
                }
            ...
            if (result) {
                ...
                if (result.expressivNo) {
                    ...
                    try {
                        const reimResponse = await makeAuthenticatedRequest(`/api/ar-invoice-clone/${result.expressivNo}`, {
                            method: 'GET',
                            headers: {
                                'Content-Type': 'application/json'
                            }
...
async function loadAttachmentsFromAPI(docId) {
    try {
        ...
        const response = await makeAuthenticatedRequest(`/api/staging-ar-invoice-clone/attachments/${docId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
        ...
        if (!response.ok) {
            ...
            if (response.status === 405) {
                ...
                // Try to get attachments from the main document endpoint
                const mainResponse = await makeAuthenticatedRequest(`/api/staging-ar-invoice-clone/headers/${docId}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json'
                    }
...
async function loadReimbursementAttachments(reimbursementId) {
    try {
        ...
        const response = await makeAuthenticatedRequest(`/api/ar-invoice-clone/${reimbursementId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
