...
// Helper function to check if makeAuthenticatedRequest is available
function checkAuthFunction() {
    if (typeof makeAuthenticatedRequest !== 'function') {
        throw new Error('makeAuthenticatedRequest function is not available. Please ensure auth.js is loaded.');
    }
}

// Initialize page function
function initializePage() {
    console.log('Initializing AR Invoice detail page...');

    // Test backend connection first
    testBackendConnection().then(() => {
        // Load document data when page is ready
        loadDocumentData();
    }).catch(error => {
        console.error('Backend connection test failed:', error);
        // Still try to load document data
        loadDocumentData();
    });
}
...
async function testBackendConnection() {
    ...
    for (const baseUrl of possibleBaseUrls) {
        try {
            const testUrl = `${baseUrl}/api/health`; // Common health check endpoint
            ...
            if (response.ok) {
                console.log('✅ Backend is reachable at:', baseUrl);
                // Update BASE_URL to the working one
                window.BASE_URL = baseUrl;
                return;
            } else {
                console.warn('⚠️ Backend responded with status:', response.status, 'at:', baseUrl);
            }
        } catch (error) {
            console.warn('⚠️ Backend not reachable at:', baseUrl, error.message);
        }
    }

    console.warn('⚠️ No backend connection found, using default BASE_URL');
}
...
function mapResponseToForm(data) {
    ...
    // Display Print Out AR Invoice document
    console.log('🖨️ Displaying Print Out AR Invoice...');
    if (typeof displayPrintOutARInvoice === 'function') {
        displayPrintOutARInvoice(data);
    } else {
        console.warn('displayPrintOutARInvoice function not found');
    }
...
async function loadDocumentData() {
    ...
    if (docId) {
        ...
        try {
            ...
            // Check if makeAuthenticatedRequest is available
            checkAuthFunction();
            ...
            for (const endpoint of endpoints) {
                try {
                    console.log('🌐 Trying API Request:', `GET ${endpoint}`);
                    console.log('🔗 Full URL:', `${typeof BASE_URL !== 'undefined' ? BASE_URL : 'http://localhost:5249'}${endpoint}`);
            ...
            let errorMessage = `Failed to load document: ${error.message}`;

            // Add more specific error messages
            if (error.message.includes('404')) {
                errorMessage = `Document not found (404). Please check if the document ID is correct: ${docId}`;
                ...
                errorMessage = `All API endpoints failed. Please check server connection and document ID: ${docId}`;
            }
...
window.displayPrintOutARInvoice = displayPrintOutARInvoice;
window.checkAuthFunction = checkAuthFunction; 