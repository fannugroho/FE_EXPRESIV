...
function selectReimbursement(docId, voucherNo) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            // Redirect ke halaman pembuatan AR Invoice dengan parameter reimbursement ID
            window.location.href = `../AddARin/add_ARInvoiceNew.html?reimbursement-id=${docId}`;
        }
    });
...
function detailDoc(opId) {
    // Navigate to AR Invoice detail page
    // Use absolute path to avoid path resolution issues
    const currentPath = window.location.pathname;
    const basePath = currentPath.substring(0, currentPath.indexOf('/0_Menu/'));
    const detailUrl = `${basePath}/0_DetailCreated/detailARInvoiceNew.html?id=${opId}`;
    
    debugNavigation('detailDoc', detailUrl);
    console.log('Current location:', window.location.href);
