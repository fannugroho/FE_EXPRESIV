...
async function fetchInvoiceData() {
    try {
        // Show loading state
        document.getElementById('recentDocs').innerHTML = '<tr><td colspan="10" class="text-center py-4">Loading data...</td></tr>';
        
        // Fetch data from API
        const response = await fetch('https://expressiv-be-sb.idsdev.site/api/ar-invoices');
        ...
        document.getElementById('recentDocs').innerHTML = `<tr><td colspan="10" class="text-center py-4 text-red-500">Error loading data: ${error.message}</td></tr>`;
    }
}
...
function getStatusFromInvoice(invoice) {
    ...
    // In a real implementation, you would check specific fields in the invoice object
    ...
    // For demo purposes, determine status based on other fields
    // In real implementation, you would have proper status fields from the API
    if (invoice.u_BSI_Expressiv_IsTransfered === 'Y') return 'Received';
    
    // Simplified logic for demo - in real app, use actual status fields
    return invoice.stagingID.includes('STG') ? 'Draft' : 'Prepared';
}
...
function displayInvoices() {
    ...
    if (pageInvoices.length === 0) {
        tableBody.innerHTML = `<tr><td colspan="10" class="text-center py-4">No invoices found</td></tr>`;
        return;
    }
    ...
    pageInvoices.forEach((invoice, index) => {
        ...
        // Format date
        const docDate = new Date(invoice.docDate);
        ...
        row.innerHTML = `
            ...
            <td class="p-2 border-b">${invoice.docType === 'I' ? 'Invoice' : 'Credit Memo'}</td>
            <td class="p-2 border-b">
                <div class="flex space-x-1">
                    <button onclick="viewInvoiceDetails('${invoice.stagingID}')" class="p-1 bg-blue-500 text-white rounded hover:bg-blue-600">
                        <i class="fas fa-eye"></i>
                    </button>
                    <button onclick="editInvoice('${invoice.stagingID}')" class="p-1 bg-yellow-500 text-white rounded hover:bg-yellow-600">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button onclick="printInvoice('${invoice.stagingID}')" class="p-1 bg-green-500 text-white rounded hover:bg-green-600">
    ...
    if (!query) {
        ...
        filteredInvoices = allInvoices.filter(invoice => {
            switch (type) {
                ...
                case 'date':
                    const date = new Date(invoice.docDate).toLocaleDateString('en-GB').toLowerCase();
...
function viewInvoiceDetails(id) {
    // Redirect to detail page with invoice ID
    ...
    try {
        ...
        const excelData = dataToExport.map((invoice, index) => {
            ...
            return {
                ...
                'Type': invoice.docType === 'I' ? 'Invoice' : 'Credit Memo'
            };
        });
    ...
    try {
        ...
        const pdfData = dataToExport.map((invoice, index) => {
            ...
            return [
                ...
                invoice.docType === 'I' ? 'Invoice' : 'Credit Memo'
            ];
        });
