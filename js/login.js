  ...
  fetch(`${BASE_URL}/api/authentication/login`, {
    ...
    .then(result => {
      ...
      if (result.status && result.code === 200) {
        ...
        // Try to get employee data from different possible property names
        const employee = result.data.employee || result.data.Employee;

        console.log("Employee data from response:", employee);
        ...
        if (employee) {
          ...
          // Store individual employee fields for backward compatibility
          if (employee.kansaiEmployeeId || employee.KansaiEmployeeId) {
            localStorage.setItem("kansaiEmployeeId", employee.kansaiEmployeeId || employee.KansaiEmployeeId);
          }
          if (employee.code || employee.Code) {
            localStorage.setItem("employeeCode", employee.code || employee.Code);
          }

          const department = employee.department || employee.Department;
          if (department && (department.name || department.Name)) {
            localStorage.setItem("departmentName", department.name || department.Name);
          }
        } else {
        ...
        if (userInfo) {
          ...
          const userObject = {
            ...
            employee: employee // Include employee data in the user object
          };
