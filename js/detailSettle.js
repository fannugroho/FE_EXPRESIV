...
async function getAvailableCategories(departmentId, transactionType) {
    ...
    try {
        ...
        if (window.expensesCoaServiceWithFallback && typeof window.expensesCoaServiceWithFallback.getAvailableCategories === 'function') {
            ...
            return await window.expensesCoaServiceWithFallback.getAvailableCategories(departmentId, 'Settlement', transactionType);
        } else {
...
async function getAvailableAccountNames(category, departmentId, transactionType) {
    ...
    try {
        ...
        if (window.expensesCoaServiceWithFallback && typeof window.expensesCoaServiceWithFallback.getAvailableAccountNames === 'function') {
            ...
            return await window.expensesCoaServiceWithFallback.getAvailableAccountNames(category, 'Settlement', transactionType, departmentId);
        } else {
            console.warn('Expenses COA service not available, using fallback API for account names');
            // Fallback to old API if service is not available
            const response = await fetch(`${BASE_URL}/api/expenses/accountnames?category=${encodeURIComponent(category)}&departmentId=${departmentId}&menu=Cash Advance Settlement&transactionType=${transactionType}`);
            if (!response.ok) {
...
async function getCOA(category, accountName, departmentId, transactionType) {
    ...
    try {
        ...
        if (window.expensesCoaServiceWithFallback && typeof window.expensesCoaServiceWithFallback.getCOA === 'function') {
            ...
            return await window.expensesCoaServiceWithFallback.getCOA(category, accountName, 'Settlement', transactionType, departmentId);
        } else {
